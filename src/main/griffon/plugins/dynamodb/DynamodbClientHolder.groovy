 ...
 */
@Singleton
class DynamodbClientHolder implements DynamodbProvider {
    ...
    AmazonDynamoDBClient getClient(String clientName = 'default') {
    ...
    Object withDynamodb(String clientName = 'default', Closure closure) {
        AmazonDynamoDBClient client = fetchClient(clientName)
    ...
    private AmazonDynamoDBClient fetchClient(String clientName) {
        if(isBlank(clientName)) clientName = 'default'
        AmazonDynamoDBClient client = retrieveClient(clientName)
        if(client == null) {
            GriffonApplication app = ApplicationHolder.application
            ConfigObject config = DynamodbConnector.instance.createConfig(app)
            client = DynamodbConnector.instance.connect(app, config, clientName)
        }
        
        if(client == null) {
            throw new IllegalArgumentException("No such AmazonDynamoDBClient configuration for name $clientName")
        }
        client
    }

    private AmazonDynamoDBClient retrieveClient(String clientName) {
        synchronized(LOCK) {
            clients[clientName]
        }
    }

    private void storeClient(String clientName, AmazonDynamoDBClient client) {
        synchronized(LOCK) {
